...
in
{
  ...
  services.vaultwarden = {
    ...
    config = {
      ...
      # ROCKET_TLS.certs = "/etc/vaultwarden/.tls/vaultwarden.local.pem"; # in env
      # ROCKET_TLS.key = "/etc/vaultwarden/.tls/vaultwarden.local-key.pem";  # in envà
      # IN ENV : ROCKET_TLS={certs="/etc/vaultwarden/.tls/vaultwarden.local.pem",key="/etc/vaultwarden/.tls/vaultwarden.local-key.pem"}
    };
  };
}
